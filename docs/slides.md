...
Note:
這段是你們最缺、別人最少講的。其實工具早就在手上了 ── 公司有提供 GitHub Copilot 訂閱（token 有限），而且現在的 agent 連「記憶」都幫你內建好了（CLAUDE.md / 記憶檔），門檻比你想的低。但現況是：很多人還不太熟怎麼用，或停在「開個對話框問它」的 chat 階段；就算有在用的，多半也還沒透過 CLI agent 把效益拉到最大。
...
<li><strong>memory</strong> ✅　agent 自動處理（CLAUDE.md / 記憶檔）</li>
