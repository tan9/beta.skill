...
Note:
這段是你們最缺、別人最少講的。其實工具早就在手上了 ── 公司有提供 GitHub Copilot 訂閱（token 有限），而且現在的 agent 連「記憶」都幫你內建好了（會把跨 session 的事實寫進記憶檔），門檻比你想的低。但現況是：很多人還不太熟怎麼用，或停在「開個對話框問它」的 chat 階段；就算有在用的，多半也還沒透過 CLI agent 把效益拉到最大。
...
<ul class="scorecard">
<li><strong>skills</strong> ✅　beta.skill</li>
<li><strong>connectors</strong> ✅　az / gh / aws / kubectl / grafana</li>
<li><strong>sub-agents</strong> ✅　那把尺（測試 / 驗證）</li>
<li><strong>worktrees</strong> ✅　多版本平行維護（靠 git 基本功）</li>
<li><strong>memory</strong> ✅　agent 記憶檔自動累積（auto-memory）</li>
<li><strong>automations</strong> ✕　龍蝦（OpenClaw）死了 ── 訂閱禁程式介接</li>
</ul>

Note:
六塊踩到五塊 ── 而且 memory 這塊坦白說不是我厲害，是 agent 自動幫你做的：Claude Code 會把跨 session 的事實、決策寫進記憶檔（auto-memory），下次自己讀回來，你不用重講。（精確一點：CLAUDE.md / AGENTS.md / copilot-instructions 那種是「專案說明、慣例」，算 context、不算 memory ── memory 是會累積的「狀態」。）
